...
export default function GameCanvas({
  ...
  // --- AUTHORITATIVE GAME STATE SYNC (ALL CONNECTED PLAYERS) ---
  useEffect(() => {
    if (gameMode !== 'ONLINE' || !lobbyId || !effectivePlayerId) return;
  ...
  return (
    <div className="w-full flex-1 flex flex-col justify-center items-center relative h-full bg-[#0C0C14] font-sans p-0 select-none">
      
      {/* Dynamic Arcade/Console Frame styled with industrial fluorescent steel and kitchen deep blue */}
      <div 
        className="w-full max-w-[1920px] [--enclosure-max-h:78vh] sm:[--enclosure-max-h:82vh] lg:[--enclosure-max-h:86vh] max-h-[78vh] sm:max-h-[82vh] lg:max-h-[86vh] bg-[#0C0C14] rounded-3xl border-8 sm:border-12 border-[#1A1A2E] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.9)] relative overflow-hidden flex items-center justify-center transition-all duration-300"
        style={{ 
          aspectRatio: gameMode === 'ONLINE' ? '16 / 9' : `${consoleWidth} / ${consoleHeight}`,
          maxWidth: gameMode === 'ONLINE' ? 'calc(var(--enclosure-max-h) * 16 / 9)' : `calc(var(--enclosure-max-h) * ${consoleWidth} / ${consoleHeight})`
        }}
      >
        
        {/* Playfield Area */}
        <div className="relative w-full h-full bg-[#070605] overflow-hidden">
          <canvas
            ref={canvasRef}
            className="absolute top-[58px] bottom-0 left-0 right-0 object-contain shadow-inner cursor-default block"
            style={{ imageRendering: 'pixelated', width: '100%', height: 'calc(100% - 58px)' }}
          />

          {/* Player 1 Smooth Touch controls Overlay */}
          {showTouchControls && !showTaskAssignment && !isPaused && !isGameOver && (
            <>
              {/* Left Side: Virtual Joystick (Joystick táctil) */}
              <div className={`absolute bottom-6 left-6 z-20 flex flex-col items-center select-none pointer-events-auto transition-all duration-200 touch-none ${countdown > 0 ? 'opacity-60' : (joystickActive ? 'opacity-100 scale-105' : 'opacity-85 hover:opacity-100')}`}>
                <div 
                  ref={joystickRef}
                  onTouchStart={handleJoystickStart}
                  onMouseDown={handleJoystickMouseDown}
                  className="w-32 h-32 sm:w-36 sm:h-36 bg-[#141D26]/95 backdrop-blur-sm border-4 border-white shadow-[4px_4px_0px_#D31F26] rounded-full relative flex items-center justify-center cursor-grab active:cursor-grabbing transform rotate-[-2deg] touch-none select-none"
                  title="Joystick de movimiento"
                  style={{ touchAction: 'none' }}
                >
                  {/* Subtle Direction Indicators inside the Joystick Rim */}
                  <span className="absolute top-1.5 text-white/40 text-xs text-center select-none font-bold">▲</span>
                  <span className="absolute bottom-1.5 text-white/40 text-xs text-center select-none font-bold">▼</span>
                  <span className="absolute left-2 text-white/40 text-xs select-none font-bold">◀</span>
                  <span className="absolute right-2 text-white/40 text-xs select-none font-bold">▶</span>
                  
                  {/* Outer circle layout design lines */}
                  ...
                  <div
                    className={`w-14 h-14 sm:w-16 sm:h-16 bg-[#F9D014] border-3 border-white rounded-full flex flex-col items-center justify-center text-[#0C0C14] shadow-[2px_2px_0px_#0C0C14] select-none pointer-events-none absolute ${joystickActive ? 'transition-none' : 'transition-transform duration-150 ease-out'}`}
                  ...
                  >
                    ...
                    <span className="text-[7.5px] sm:text-[8px] font-black tracking-widest text-[#D31F26] uppercase select-none leading-none mt-0.5">
                      {gameMode === 'ONLINE' ? `CHEF ${myRoleNumRef.current || 1}` : 'CHEF 1'}
                    </span>
                  </div>
                </div>
              </div>

              {/* Right Side: Virtual Big Action Buttons without keyboard keys */}
              <div className={`absolute bottom-6 right-6 z-20 flex gap-2.5 sm:gap-4 items-center select-none pointer-events-auto transition-opacity duration-300 touch-none ${countdown > 0 ? 'opacity-60' : 'opacity-85 hover:opacity-100 active:opacity-100 focus-within:opacity-100'}`}>
                <div className="flex gap-2.5 sm:gap-3 bg-[#141D26]/95 backdrop-blur-sm p-2.5 sm:p-3.5 rounded-2xl border-4 border-white shadow-[4px_4px_0px_#F9D014] transform rotate-[1.5deg] skew-x-[-4deg] touch-none">
                  
                  {/* ACCION/TRABAJAR Button (Washing, chopping, grinding) */}
                  <button
                    {...getTouchKeyHandlers('e')}
                    className="w-14 h-14 sm:w-16 sm:h-16 bg-[#D31F26] hover:brightness-110 active:brightness-95 active:translate-y-0.5 border-2 border-white rounded-xl flex flex-col items-center justify-center text-white shadow-[2px_2px_0px_#0C0C14] transition-all outline-none select-none cursor-pointer transform rotate-[-2deg] touch-none"
                    style={{ touchAction: 'none' }}
                  >
                    <span className="text-base sm:text-xl">🔪</span>
                    <span className="text-[8px] sm:text-[9px] font-black tracking-tight uppercase whitespace-nowrap text-[#F9D014] mt-0.5">ACCIÓN</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-14 h-14 sm:w-16 sm:h-16 bg-[#2563EB] hover:brightness-110 active:brightness-95 active:translate-y-0.5 border-2 border-white rounded-xl flex flex-col items-center justify-center text-white shadow-[2px_2px_0px_#0C0C14] transition-all outline-none select-none cursor-pointer transform rotate-[3deg] touch-none"
                    style={{ touchAction: 'none' }}
                  >
                    <span className="text-base sm:text-xl">🫳</span>
                    <span className="text-[8px] sm:text-[9px] font-black tracking-tight uppercase whitespace-nowrap text-white mt-0.5">TOMAR</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-14 h-14 sm:w-16 sm:h-16 bg-[#F9D014] hover:brightness-110 active:brightness-95 active:translate-y-0.5 border-2 border-white rounded-xl flex flex-col items-center justify-center text-[#0C0C14] shadow-[2px_2px_0px_#0C0C14] transition-all outline-none select-none cursor-pointer transform rotate-[-1deg] touch-none"
                    style={{ touchAction: 'none' }}
                  >
                    <span className="text-base sm:text-xl">⚡</span>
                    <span className="text-[8px] sm:text-[9px] font-black tracking-tight uppercase whitespace-nowrap text-[#D31F26] mt-0.5">CORRER</span>
                  </button>
